...
public class ChaosMark extends Item {
    private final ModRarities rarity;
    public ChaosMark(Settings settings, ModRarities rarity) {
        super(settings);
        this.rarity = rarity;
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("tooltip.paradigm.chaos_mark.tooltip").formatted(Formatting.DARK_PURPLE));
        } else {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (user.isSneaking() && !world.isClient()) {
            ...
            for (LivingEntity entity : entities) {
                ...
                entity.addStatusEffect(new StatusEffectInstance(StatusEffects.STRENGTH, 600, 0));
