...
public class EmptyAfterlifeEffigy extends TrinketItem {
    public EmptyAfterlifeEffigy(Settings settings) {
        super(settings);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("tooltip.paradigm.emptyafterlifeeffigy.tooltip"));
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (state.isOf(Blocks.BONE_BLOCK) //Afterlife Effigy starter
        ) {
            if (!world.isClient()) {
