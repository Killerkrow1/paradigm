...
public class EmptyEsotericEffigy extends Item {
    ...
    private static final int REQUIRED_COUNT = REQUIRED_BLOCKS.length + 30;

    public EmptyEsotericEffigy(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!alreadyAbsorbed) {
            ...
            if (absorbedList.size() >= REQUIRED_COUNT) {
                ItemStack transformedStack = new ItemStack(ModItems.CHARGED_EMPTY_ESOTERIC_EFFIGY);
